    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4 backdrop-blur-md animate-fade-in">
      <div className="bg-white rounded-2xl w-full max-w-4xl h-auto max-h-[90vh] overflow-hidden flex flex-col md:flex-row shadow-2xl relative animate-scale-up">
        ...
        <div className="w-full md:w-1/2 bg-gray-100 flex items-center justify-center p-6 relative group overflow-hidden">
          {product.imagen ? (
            <img
              src={product.imagen && product.imagen.startsWith('http') ? product.imagen : `${API_BASE_URL}${product.imagen}`}
              alt={product.nombre}
        ...
        <div className="w-full md:w-1/2 p-8 flex flex-col overflow-y-auto">
          ...
          <div className="mt-8 pt-6 border-t border-gray-100 grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <button
              ...
              className={`flex-1 font-bold py-3.5 px-6 rounded-xl transition-all shadow-md flex items-center justify-center gap-2 border-2 ${product.objeto3D?.glbUrl
...
const CatalogPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const productsRes = await fetch(`${API_BASE_URL}/api/productos?disponibles=true`);
          ...
          <div className="flex gap-2 overflow-x-auto w-full md:w-auto pb-2 md:pb-0 custom-scrollbar">
            {availableCategories.map(cat => (
              <button
                ...
                className={`px-5 py-2 rounded-full whitespace-nowrap text-sm font-bold transition-all border ${selectedCategory === cat
