      ...
      <div className="container mx-auto px-4 py-16">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="bg-gray-800 rounded-lg p-8 shadow-2xl">
            ...
            {logisticaInfo && (
              <div className="mt-6 p-4 bg-gray-700 rounded-lg border border-gray-600">
                <h3 className="text-lg font-bold text-blue-400 mb-3">Información de Logística</h3>
                <div className="space-y-2 text-sm">
                  {logisticaInfo.transportista && (
                    <p><span className="font-medium text-gray-300">Transportista:</span> {logisticaInfo.transportista.nombre}</p>
                  )}
                  {logisticaInfo.ruta && (
                    <p><span className="font-medium text-gray-300">Ruta:</span> {logisticaInfo.ruta.nombre}</p>
                  )}
                  {logisticaInfo.fechaSalida && (
                    <p><span className="font-medium text-gray-300">Fecha de Salida:</span> {new Date(logisticaInfo.fechaSalida).toLocaleDateString()}</p>
                  )}
                  {logisticaInfo.fechaLlegada && (
                    <p><span className="font-medium text-gray-300">Fecha de Llegada:</span> {new Date(logisticaInfo.fechaLlegada).toLocaleDateString()}</p>
                  )}
                  {logisticaInfo.estado && (
                    <p><span className="font-medium text-gray-300">Estado de Envío:</span> {logisticaInfo.estado}</p>
                  )}
                  {logisticaInfo.notas && (
                    <p><span className="font-medium text-gray-300">Notas:</span> {logisticaInfo.notas}</p>
                  )}
                </div>
              </div>
            )}
